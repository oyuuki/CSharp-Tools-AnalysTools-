...
{
    class InputItemVBSourceGenerater : InputItemGenerater
    {
        #region constractor

        public InputItemVBSourceGenerater()
        {

        }

        public InputItemVBSourceGenerater(string sourceText, int hierarchyIndex, string itemSignature)
        ...
        protected override string GetImeMode()
        {
            ...
            if (this.GetExType().IndexOf("imDate") >= 0
                    || this.GetExType().IndexOf("imNumber") >= 0)
            {
                return ConstAttributeManager<EnumImeMode>.GetConstByEnumValue(EnumImeMode.Off);
            }
